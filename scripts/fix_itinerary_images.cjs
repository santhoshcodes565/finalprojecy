...
async function fixImages() {
  ...
  // Second logic block using pure regex replacement which is safer:
  content = content.replace(/(itinerary:\s*\[)([\s\S]*?)(\n\s*\])/g, (match, prefix, items, suffix) => {
    let updatedItems = items.replace(/{([\s\S]*?)}/g, (objMatch, objBody) => {
        let cat = 'generic';
        const bl = objBody.toLowerCase();
        
        if (bl.match(/beach|kovalam|marari|sea|coast|goa/)) cat = 'beach';
        else if (bl.match(/hill|munnar|wayanad|ooty|kodaikanal|valparai|tea|yercaud|darjeeling|sikkim|valley/)) cat = 'hills';
        else if (bl.match(/waterfall|dudhsagar|jog|falls/)) cat = 'waterfall';
        else if (bl.match(/temple|madurai|thanjavur|rameshwaram|kanyakumari|church|mosque|architecture|monument|gopuram|shrine/)) cat = 'temple';
        else if (bl.match(/boat|houseboat|backwater|alleppey|lake|kumarakom|river|cruise/)) cat = 'boat';
        else if (bl.match(/fort|palace|mysore|mahal|mansion/)) cat = 'palace';
        else if (bl.match(/safari|wildlife|thekkady|park|sanctuary|tiger|forest|elephant/)) cat = 'safari';
        else if (bl.match(/city|arrival|departure|airport|chennai|kochi|bangalore|trivandrum|delhi|mumbai/)) cat = 'city';
        
        let newImg = getImg(cat);
        // Using standard string concatenation so no syntax error:
        return objMatch.replace(/image:\s*['"][^'"]+['"]/, "image: '" + newImg + "'");
    });
    return prefix + updatedItems + suffix;
  });

  fs.writeFileSync(file, content, 'utf8');
  console.log('Fixed itinerary images!');
}
